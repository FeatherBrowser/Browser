...
function renderState(state) {
  if (state.favicons && typeof state.favicons === "object") {
    const changed = JSON.stringify(favicons) !== JSON.stringify(state.favicons);
  ...
  for (const [id, key] of [
    ...
    if (element) {
      element.setAttribute("aria-checked", String(Boolean(state[key])));
    }
  }
...
function renderLinks() {
  ...
  links.forEach((link, index) => {
    ...
    const icon = makeIcon(link.Url, link.Name, "site-icon");
...
function renderRecent() {
  ...
  recentItems.slice(0, 2).forEach((item) => {
    ...
    const icon = makeIcon(item.url, item.title, "recent-icon");
    ...
    title.textContent = item.title || item.host || "Recent page";

    const host = document.createElement("span");

    host.className = "recent-host";
    host.textContent = item.host || hostFor(item.url);
    ...
    more.setAttribute("aria-label", `Options for ${title.textContent}`);
    ...
    remove.addEventListener("click", (event) => {
      ...
      recentItems = recentItems.filter((entry) => entry.url !== item.url);
    ...
    more.addEventListener("click", (event) => {
      ...
      document.querySelectorAll(".recent-menu").forEach((other) => {
        if (other !== menu) {
          other.hidden = true;
        }
      });
    ...
    card.append(icon, copy, more, menu);
...
function openEditor(index) {
  ...
  document.getElementById("dialog-title").textContent =
    index < 0 ? "Add shortcut" : "Edit shortcut";

  document.getElementById("delete-link").hidden = index < 0;

  nameInput.value = index < 0 ? "" : links[index].Name;

  urlInput.value = index < 0 ? "" : links[index].Url;
...
function setEditing(value) {
  ...
  document
    ...
    .querySelector("span:last-child").textContent = editing
    ? "Done editing"
    : "Edit shortcuts";
...
document.getElementById("delete-link").addEventListener("click", () => {
  ...
  saveLinks(links.filter((_, index) => index !== selected));
});
...
document.getElementById("shortcut-form").addEventListener("submit", (event) => {
  ...
  try {
    ...
    error.textContent = "Enter a valid HTTP or HTTPS website.";

    return;
  }

  if (!name) {
    error.textContent = "Give this shortcut a name.";
